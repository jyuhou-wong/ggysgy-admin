  ...
  <div>
        ...
        <el-dropdown class="crpr margin_l_50" @command="handleCommand">
          ...
          <el-dropdown-menu slot="dropdown">
            <el-dropdown-item class="el-icon-edit-outline dflex" command="pwdChange"
              ><div class="margin_l_5">修改密码</div></el-dropdown-item
            >
            <el-dropdown-item class="el-icon-switch-button dflex" command="logOut"
              ><div class="margin_l_5">安全退出</div></el-dropdown-item
            >
          </el-dropdown-menu>
        </el-dropdown>
      </div>
    ...
    <el-drawer title="通知" :visible.sync="informDrawer" direction="rtl" :append-to-body="true">
      <el-tooltip
        ...
        :enterable="false"
        effect="light"
        v-for="(item, i) in informDatas"
        :key="i"
      >
        <div
          v-if="!item.read"
          :ref="item.ref"
          class="inform crpr margin_lr_20 margin_b_20 padding_20 animated"
          :class="{ fadeOut: item.read == 1 }"
          @dblclick="read(i)"
        >
