<template>
  <div class="tab" ref="tabChange">
    <div class="dflex h100">
      <div
        class="tab_content dflex_c h100 margin_r_10 crpr"
        v-for="(item, i) in tagsList"
        :class="{ active: isActive(item.path) }"
        :key="i"
      >
        <router-link :to="item.path" class="tab_name h100 dflex_c">{{ item.title }}</router-link>
        <span class="el-icon-close padding_r_5" v-if="item.close" @click="closeTags(i)"></span>
      </div>
    </div>
      ...
      setTags(route) {
        ...
        if (!isExist) {
          ...
          if (menus && menus.length > 0) {
            const menu = menus.find((x) => x.module_url == route.path) || { name: route.query.tab };
            if (menu) {
              this.tagsList.push({
