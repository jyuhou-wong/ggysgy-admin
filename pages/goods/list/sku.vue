      ...
      <div>
        ...
        <div>
          <el-form-item label="开启SKU">
            <el-switch @change="open_switch_sku" v-model="switch_sku" active-color="#feaa30" inactive-color="#dcdfe6"></el-switch>
          </el-form-item>
          <el-form-item label="编辑SKU" v-if="switch_sku">
            ...
            <div class="dflex margin_t_20">
              <el-tag
                ...
                class="tag-new-tag"
                :key="tag"
                v-for="tag in dynamicTags_spec"
                :closable="checked_spec"
                :disable-transitions="false"
                @close="handleClose_spec(tag)"
              >
                {{ tag }}
              </el-tag>
              <el-input
                class="input-new-tag"
                v-if="inputVisible_spec"
                v-model="inputValue_spec"
                ref="saveTagInput_spec"
                @keyup.enter.native="handleInputConfirm_spec"
                @blur="handleInputConfirm_spec"
              ></el-input>
              <el-button :disabled="!checked_spec" v-else class="button-new-tag" size="small" @click="showInput_spec">添加</el-button>
            </div>
          </el-form-item>
            ...
            <el-table :data="tableSkuData" border>
              <el-table-column label="价格" align="center">
                <template slot-scope="scope">
                  <el-input class="sort-container" type="number" v-model.number="scope.row.price"></el-input>
                </template>
              </el-table-column>
              <el-table-column label="库存" align="center">
                <template slot-scope="scope">
                  <el-input class="sort-container" type="number" v-model.number="scope.row.stock_num"></el-input>
                </template>
              </el-table-column>
              <el-table-column v-if="checked_spec" prop="spec" :label="spec_s" align="center"></el-table-column>
            </el-table>
          </el-form-item>
        </div>
      </div>
...
export default {
  data() {
    return {
      ...
      skus: [],
      goods: {},
      dynamicTags_spec: ['电子版(个人授权)', '纸质版(个人授权)', '电子版(商业授权)', '纸质版(商业授权)'],
