    ...
    <div class="container use-table">
      <el-table :height="tblHeight" :data="tableDatas" highlight-current-row>
            ...
            <div class="dflex">
              <div class="margin_r_10" v-for="(item, i) in scope.row.review_imgs" :key="i">
                <el-image
                  ...
                  :preview-src-list="scope.row.review_imgs"
                  :src="item"
                  fit="contain"
                ></el-image>
              </div>
        ...
        <el-table-column label="状态" align="center" width="100">
          <template slot-scope="scope">
            <el-tooltip
              :content="scope.row.state == '显示' ? '隐藏' : '显示'"
              placement="top"
              :hide-after="1000"
              :enterable="false"
              effect="light"
    ...
    <el-drawer title="回复内容" :visible.sync="replyDrawer" direction="rtl" :append-to-body="true">
      <el-form class="add_edit" :model="form" :rules="rules" ref="ruleForm" label-width="100px">
        <el-form-item label="回复内容" prop="reply" ref="reply"
          ><el-input type="textarea" :autosize="{ minRows: 3 }" v-model="form.reply"></el-input
        ></el-form-item>
        <el-form-item>
