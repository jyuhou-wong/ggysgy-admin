    ...
    <div class="container padding_b_0">
      <div class="dflex_wrap">
        <div class="dflex_vertical_c margin_r_40 margin_b_20">
          ...
          <el-input v-model="req.username" placeholder="请输入" @input="loadData" class="search_input"></el-input>
        </div>
    ...
    methods: {
      async loadData() {
        await this.$db[__name]
          .where({ role: this.$db.$cmd.neq('member') })
          .whereif(this.req.username, { username: new RegExp(this.req.username) })
          .whereif(this.req.gender, { gender: this.req.gender })
