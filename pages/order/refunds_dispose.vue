  ...
  <div>
            ...
            <div v-if="scope.row.state == '处理中'">
              <el-tooltip content="同意退款" placement="top" :hide-after="1000" :enterable="false" effect="light">
                <el-button
                  ...
                  icon="el-icon-check"
                  class="bbb"
                  @click="refunded(scope.row._id, scope.row.order_id)"
                ></el-button>
              </el-tooltip>
              <el-tooltip content="拒绝退款" placement="top" :hide-after="1000" :enterable="false" effect="light">
                <el-button
                  ...
                  icon="el-icon-close"
                  class="bbb"
                  @click="refuseRefund(scope.row._id, scope.row.order_id)"
                ></el-button>
              </el-tooltip>
            </div>
          </template>
    ...
    >
      <el-form class="add_edit" :model="form" :rules="rules" ref="ruleForm" label-width="100px">
        <el-form-item label="拒绝原因" prop="refund" ref="refund"
          ><el-input type="textarea" :autosize="{ minRows: 3 }" v-model="form.refund"></el-input
        ></el-form-item>
        <el-form-item>
    ...
    methods: {
      ...
      refunded(id, orderId) {
        this.$confirm('此操作将同意退款！', '提示', {
          ...
          this.$func.useadmin
            .call('order/refund', { id: id, order_id: orderId, remark: '退款成功', state: '已退款' })
            .then((res) => {
              if (res.code == 200) {
                this.loadData();
              }
